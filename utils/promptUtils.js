  ...
  if (use_node_ai && node) {
    ...
    if (isNodeConfigured(settings)) {
      ...
      if (nodeInfo.name.toLowerCase() === "midjourney") {
        ...
        if (mjPrompt) {
          node_input = mjPrompt.prompt.replaceAll("--", " ") + " " + mjPrompt.parameters;
        }
      }
      ...
      if (queryResult && queryResult.success) {
        ...
        if (typeof queryResult.result === "string") {
          ...
          if (queryResult.result.images && queryResult.result.images.length > 0) {
            ...
            // Add system message
            const node_ai_image_system_prompt = "Image generated by user, please check it out.";
            messages.push({
              "role": "system",
              "content": node_ai_image_system_prompt
            });

            // add to node prompt for token count
            node_prompt += node_output + node_ai_image_system_prompt;
