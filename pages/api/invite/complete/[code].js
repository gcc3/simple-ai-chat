...
export default async function (req, res) {
  ...
  try {
    ...
    if (!authResult.success) {
      res.status(401).send(getRedirectableHtml("Please login first before using the invitation link. To register a user, use the command \`:user add [username] [email] [password?]\` (no bracktes needed)."));
      return;
    }
