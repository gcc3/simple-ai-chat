...
export default async function (req, res) {
  ...
  if (process.env.USE_EMAIL == "true") {
    ...
    const body = "Your account is created successfully." + (!password ? " Initial password is \"" + generatedPassword + "\", please change it after login." : "") 
               + "<br><br>" + `Please click the following link to verify your email: <a href="https://simple-ai.io/api/verify-email/${token}">https://simple-ai.io/api/verify-email/${token}</a>`;
    const emailParams = {
