...
import { getFunctions } from '../ai/function.js';
...
const Documentation = () => {
  ...
  const features = [
    ...
    { name: t("Page redirection"), description: t("As one of the `functions calls`, `redirect_to_url()` can redirect or open URL in a new tab. GPT will do it automatically, for example: Open the official website of OpenAI. You can use it to open multiple URLs, simultaneously.") },
  ...
  const shortcuts = [
    ...
    { action: t("Clear input and output. (`:clear`)"), shortcut: "⌃R", condition: "-" },
    { action: t("Create a new session. Clear input and output, and reset session. (`:clear` and `:reset`)"), shortcut: "⌃N", condition: "-" },
